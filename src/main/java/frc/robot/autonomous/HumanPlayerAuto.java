...
public class HumanPlayerAuto extends Command {
      ...
      public void execute() {
        switch(currentState) {
            ...
            case WAIT_AT_HUMAN:
                ...
                break;
            case ALIGN_TO_SHOOT:
                drive.setStateAutoAlign(() -> (new Pose2d(shootingPose.getX(), shootingPose.getY(),
                    Rotation2d.fromDegrees(RobotState.getInstance().getShootingInfo().get(2)))));
                climber.setState(ClimberStates.CLOSE_NO_CLIMB);
                hood.setTargetAngle(() -> RobotState.getInstance().getShootingInfo().get(0));
                beltDrive.setState(BeltDriveStates.IDLE);
                hopper.setState(HopperStates.IDLE);
                shooter.runExitVelocity(() -> RobotState.getInstance().getShootingInfo().get(1));
                leds.setState(ledsStates.PURPLE);
                if (drive.isAtAlignSetpoint(0.05, 1) && (Constants.currentMode == Constants.Mode.SIM || (shooter.atVelocity() && hood.atSetpoint()))) {
                    currentState = AutoSteps.SHOOT;
                    timestamp = Timer.getTimestamp();
                }
                break;
            case SHOOT:
                    drive.setState(DriveStates.SHOOT_DRIVE);
                    hood.setTargetAngle(() -> RobotState.getInstance().getShootOnTheMoveScoringInfo().get(0));
                    shooter.runExitVelocity(() -> RobotState.getInstance().getShootOnTheMoveScoringInfo().get(1));
